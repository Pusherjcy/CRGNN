...
class EmbeddingLayer(nn.Module):
    ...
    def forward(self, idx):
        if torch.any(idx >= self.n_poi) or torch.any(idx < 0):
            raise IndexError(f"Embedding index out of range. Index values: {idx}")
        return self.embeds(idx)


class GeoGraph(nn.Module):
    def __init__(self, n_user, n_poi, gcn_num, embed_dim, edge_index, edge_attr, dist_vec, device):
        ...
        self.edge_index = torch.cat(
            (self.edge_index, self.edge_index[[1, 0]], loop_index), dim=-1
        )
        ...
        self.edge_attr = torch.cat(
            (self.edge_attr, self.edge_attr, torch.zeros(loop_index.shape[1], dtype=self.edge_attr.dtype))
        ).to(device)
        ...
        self.gcn = nn.ModuleList([
            Geo_GCN(embed_dim, embed_dim, device) for _ in range(gcn_num)
        ])
    ...
    def forward(self, data, poi_embeds):
        ...
        edge_features = self.edge_mlp(self.edge_attr.unsqueeze(-1).float())

        for i in range(self.gcn_num):
            enc = self.gcn[i](enc, self.edge_index, self.dist_vec, edge_features)
...
class Geo_GCN(nn.Module):
    ...
    def forward(self, x, edge_index, dist_vec, edge_features):
        row, col = edge_index

        dist_weight = torch.exp(-dist_vec ** 2)
        edge_weight = edge_features.sum(dim=-1) * dist_weight
        ...
        sparse_matrix = torch.sparse_coo_tensor(edge_index, edge_weight * norm, (x.size(0), x.size(0)))
        out = torch.sparse.mm(sparse_matrix, x)
        return self.W(out)
