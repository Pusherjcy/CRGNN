...
ARG.add_argument('--data', type=str, default='tky', help='Training dataset.')
ARG.add_argument('--gpu', type=int, default=None, help='Denote training device.')
ARG.add_argument('--patience', type=int, default=140, help='Early stopping patience.')
ARG.add_argument('--embed', type=int, default=64, help='Embedding dimension.')
ARG.add_argument('--gcn_num', type=int, default=2, help='Num of GCN.')
ARG.add_argument('--max_step', type=int, default=2, help='Steps of random walk.')
ARG.add_argument('--hid_graph_num', type=int, default=16, help='Num of hidden graphs.')
ARG.add_argument('--hid_graph_size', type=int, default=10, help='Size of hidden graphs')
ARG.add_argument('--weight_decay', type=float, default=5e-4, help='Weight decay rate')
ARG.add_argument('--lr', type=float, default=1e-3, help='Learning rate.')
ARG.add_argument('--log', type=str, default='C:/Users/JianQiu/Desktop/t/0606.log', help='Log file path.')
ARG.add_argument('--con_weight', type=float, default=0.01, help='Weight of consistency loss')
ARG.add_argument('--compress_memory_size', type=int, default=12800, help='Memory bank size')
ARG.add_argument('--compress_t', type=float, default=0.01, help='Softmax temperature')
...
def train_test(tr_set, va_set, te_set, arg, dist_edges, edge_attr, dist_vec, device):
    Seq_encoder = SeqGraph(n_user, n_poi, arg.max_step, arg.embed, arg.hid_graph_num, arg.hid_graph_size, device).to(
        device)
    Geo_encoder = GeoGraph(
        n_user=n_user,
        n_poi=n_poi,
        gcn_num=arg.gcn_num,
        embed_dim=arg.embed,
    ...
    Sim_criterion = CompReSSMomentum(arg.embed, arg.compress_memory_size, arg.compress_t, device).to(
        device)

    opt = torch.optim.Adam([
        {'params': Seq_encoder.parameters()},
        {'params': Geo_encoder.parameters()},
        {'params': Poi_embeds.parameters()}], lr=arg.lr)
    ...
    for epoch in range(arg.epoch):
        Seq_encoder.train()
        Geo_encoder.train()
        pbar = tqdm(total=len(train_loader), desc=f"Epoch {epoch + 1}/{arg.epoch}")
        ...
        for bn, (trn_batch, bnk_batch) in enumerate(zip(train_loader, bank_loader)):
            ...
            assert torch.all(
                trn_batch.x.squeeze() < n_poi), f"Index out of range in trn_batch.x: {trn_batch.x.squeeze()}"
            assert torch.all(
                bnk_batch.x.squeeze() < n_poi), f"Index out of range in bnk_batch.x: {bnk_batch.x.squeeze()}"
        ...
        seq_auc, seq_logloss, geo_auc, geo_logloss = eval_model(Seq_encoder, Geo_encoder, Poi_embeds, va_set, arg,
                                                                device)
        logging.info('')
        ...
        if (seq_auc > best_auc or geo_auc > best_auc):
            ...
            best_epoch = epoch
            bst_seq_auc, bst_seq_logloss, bst_geo_auc, bst_geo_logloss = eval_model(Seq_encoder, Geo_encoder,
                                                                                    Poi_embeds, te_set, arg, device)
...
if __name__ == '__main__':
    ...
    with open('E:/Dataset/FinalExperiment_NewYork/KG_file/csv_file_0528/processed/val.pkl', 'rb') as f:
        tmp = pickle.load(f)
        n_user, n_poi = pickle.load(f)
        del tmp

    train_set = MultiSessionsGraph('../processed/', phrase='train')
    val_set = MultiSessionsGraph('../processed/', phrase='test')
    test_set = MultiSessionsGraph('../processed/', phrase='val')

    with open('../processed/dist_graph.pkl', 'rb') as f:
        dist_data = pickle.load(f)

    dist_edges = [dist_data['edges'][0], dist_data['edges'][1]]
    dist_edges_tensor = torch.LongTensor(dist_edges)

    dist_attr_tensor = torch.FloatTensor(dist_data['edges'][2])

    print(f"Edges shape: {dist_edges_tensor.shape}")
    print(f"Edge attr shape: {dist_attr_tensor.shape}")

    dist_vec_sparse = load_npz('../processed/dist_on_graph.npz')

    dist_vec = dist_vec_sparse.toarray().flatten()
    ...
    train_test(train_set, test_set, val_set, ARG, dist_edges_tensor, dist_attr_tensor, dist_vec, device)
